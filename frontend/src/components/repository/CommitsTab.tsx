...
const AuthorAvatar: React.FC<{
  ...
  const color =
    colors[name?.charCodeAt(0) % colors.length] || 'bg-zinc-500';
...
export const CommitsTab: React.FC<CommitsTabProps> = ({
  ...
  const [selectedSha, setSelectedSha] =
  ...
  const {
    ...
    queryFn: async () => {
      const data = await gitApi.getCommits(owner, repo, {
        page,
        per_page: PER_PAGE,
        sha: selectedBranch,
      });
  ...
  const {
    ...
    queryFn: () =>
      gitApi.getCommit(owner, repo, selectedSha!),
  ...
  const handleRefresh = async () => {
    await qc.invalidateQueries({
      queryKey: ['git', owner, repo, 'commits'],
    });
  ...
  const filtered = useMemo(() => {
    return commitsData.filter((c: any) => {
      const message =
  ...
  const hasMore = commitsData.length === PER_PAGE;
...
const CommitRow: React.FC<{
  ...
  const { data: checks } = useQuery({
    ...
    queryFn: () =>
      gitApi.getCheckRuns(owner, repo, c.sha),
